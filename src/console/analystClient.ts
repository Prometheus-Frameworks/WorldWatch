...
export function getAnalystConsoleClientScript(): string {
  return String.raw`
    ...
    function renderScanCards(detail, explainabilitySummary, explainabilityGroups) { const container = document.getElementById('explainability-scan-cards'); if (!(container instanceof HTMLElement)) return; const mixedCount = Array.isArray(explainabilityGroups.mixed_signal_indicators) ? explainabilityGroups.mixed_signal_indicators.length : 0; const staleCount = Array.isArray(explainabilityGroups.stale_high_impact_sources) ? explainabilityGroups.stale_high_impact_sources.length : 0; const disagreementCount = Array.isArray(explainabilityGroups.source_disagreement_groups) ? explainabilityGroups.source_disagreement_groups.length : 0; const triageCount = Array.isArray(detail.triage_notes) ? detail.triage_notes.length : 0; container.innerHTML = ['<article class="scan-card"><p class="scan-label">Freshness state</p><p class="scan-value">' + String(explainabilitySummary.freshness_state ?? '-') + '</p><p class="scan-note">'+ String(explainabilitySummary.freshness_copy ?? '') + '</p></article>','<article class="scan-card"><p class="scan-label">Confidence / evidence</p><p class="scan-value">' + String(explainabilitySummary.confidence_band ?? '-') + ' / ' + String(explainabilitySummary.evidence_state ?? '-') + '</p><p class="scan-note">Trust posture at a glance.</p></article>','<article class="scan-card"><p class="scan-label">Mixed-signal domains</p><p class="scan-value">' + String(mixedCount) + '</p><p class="scan-note">Requires analyst adjudication.</p></article>','<article class="scan-card"><p class="scan-label">Stale high-impact sources</p><p class="scan-value">' + String(staleCount) + '</p><p class="scan-note">Refresh if this count is elevated.</p></article>','<article class="scan-card"><p class="scan-label">Disagreement groups</p><p class="scan-value">' + String(disagreementCount) + '</p><p class="scan-note">Check direction and reliability splits.</p></article>','<article class="scan-card"><p class="scan-label">Triage notes</p><p class="scan-value">' + String(triageCount) + '</p><p class="scan-note">Decision support notes in this snapshot.</p></article>'].join(''); }
    function renderDetail(detail) {
      ...
      renderHistoryTable('source-contributions-table', Array.isArray(detail.source_contributions) ? detail.source_contributions : [], [{ key: 'source_name', header: 'Source' }, { key: 'signal_count', header: 'Count', render: (v) => formatNum(Number(v), 0) }, { key: 'latest_event_time', header: 'Latest event', render: (v) => formatTimestamp(v) }, { key: 'avg_raw_value', header: 'Avg raw value', render: (v) => formatNum(Number(v), 2) }, { key: 'avg_reliability', header: 'Avg reliability', render: (v) => formatNum(Number(v), 2) }]);
