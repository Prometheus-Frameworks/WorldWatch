...
export function getAnalystConsoleClientScript(): string {
  return String.raw`
    ...
    function renderDetail(detail) {
      ...
      renderHistoryTable('source-contributions-table', Array.isArray(detail.source_contributions) ? detail.source_contributions : [], [{ key: 'source_name', header: 'Source' }, { key: 'signal_count', header: 'Signals', render: (v) => formatNum(Number(v), 0) }, { key: 'latest_event_time', header: 'Latest event', render: (v) => formatTimestamp(v) }, { key: 'avg_raw_value', header: 'Avg raw', render: (v) => formatNum(Number(v), 2) }, { key: 'avg_reliability', header: 'Avg reliability', render: (v) => formatNum(Number(v), 2) }]);
      ...
      renderHistoryTable('source-disagreement-table', Array.isArray(explainabilityGroups.source_disagreement_groups) ? explainabilityGroups.source_disagreement_groups.map((row) => ({ domain: row.domain, disagreement_types: Array.isArray(row.disagreement_types) ? row.disagreement_types.join(', ') : '', disagreeing_sources: Array.isArray(row.disagreeing_sources) ? row.disagreeing_sources.map((source) => source.source + ':' + source.movement_direction + ' (' + formatNum(Number(source.recency_minutes), 0) + 'm, r=' + formatNum(Number(source.source_reliability), 2) + ')').join(' | ') : '' })) : [], [{ key: 'domain', header: 'Domain' }, { key: 'disagreement_types', header: 'Disagreement type' }, { key: 'disagreeing_sources', header: 'Sources (direction, recency, reliability)' }]);
