...
export function getAnalystConsoleClientScript(): string {
  return String.raw`
    ...
    function renderDetail(detail) {
      ...
      renderHistoryTable('factors-table', Array.isArray(detail.factor_payload) ? detail.factor_payload : [], [{ key: 'signalType', header: 'Signal' }, { key: 'domain', header: 'Domain' }, { key: 'source', header: 'Source' }, { key: 'sourceReliability', header: 'Reliability', render: (v) => formatNum(Number(v), 2) }, { key: 'value', header: 'Raw', render: (v) => formatNum(Number(v), 2) }, { key: 'normalizedValue', header: 'Normalized', render: (v) => formatNum(Number(v), 1) }, { key: 'movement', header: 'Direction' }, { key: 'eventTime', header: 'Event time', render: (v) => formatTimestamp(v) }, { key: 'explainabilityNote', header: 'Reason' }]);
      ...
      renderHistoryTable('source-contributions-table', Array.isArray(detail.source_contributions) ? detail.source_contributions : [], [{ key: 'source_name', header: 'Source' }, { key: 'signal_count', header: 'Signals' }, { key: 'latest_event_time', header: 'Latest event', render: (v) => formatTimestamp(v) }, { key: 'avg_raw_value', header: 'Avg value', render: (v) => formatNum(Number(v), 2) }, { key: 'avg_reliability', header: 'Avg reliability', render: (v) => formatNum(Number(v), 2) }]);
